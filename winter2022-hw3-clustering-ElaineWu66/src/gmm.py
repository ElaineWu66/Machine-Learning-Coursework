...
class GMM():
    ...
    def _e_step(self, features):
        ...
        post_probabilities = np.zeros((n_samples, self.n_clusters))
        
        for i in range(self.n_clusters):
            post_probabilities[:,i] = self._posterior(features, i)
    ...
    def _m_step(self, features, assignments):
        ...
        for i in range(self.n_clusters):
            ...
            for feature,feature_id in zip(features,range(n_samples)):
                
                means_numerator += assignments[feature_id,i] * feature
                sigma_numerator += assignments[feature_id,i] * (feature-self.means[i])**2
                Gamma_j += assignments[feature_id,i]
            
            Wj = Gamma_j/n_samples
            MUj = means_numerator/Gamma_j
            SIGMAj = sigma_numerator/Gamma_j
            
            cluster_means.append(MUj)
            cluster_covariances.append(SIGMAj)
            cluster_weights.append(Wj)

        #turn back to numpy array
        cluster_means = np.asarray(cluster_means)
        cluster_covariances = np.asarray(cluster_covariances)
    ...
    def _log_likelihood(self, features, k_idx):
        ...
        cov = self.get_cluster_covariance(k_idx, n_features)


        likelihoods = np.zeros(n_samples)
        for i in range(n_samples):
            likelihoods[i] = multivariate_normal.logpdf(features[i],mean,cov) + np.log(self.mixing_weights[k_idx])
        ...
        return (np.log(self.mixing_weights[k_idx]) + multivariate_normal.logpdf(features, mean, cov))       
        #raise NotImplementedError()
