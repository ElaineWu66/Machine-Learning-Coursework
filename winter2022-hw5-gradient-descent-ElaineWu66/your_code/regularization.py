...
class L1Regularization(Regularization):
    ...
    def forward(self, w):
        ...
        Returns:
            regularization_term - (float) The value of the regularization term
                evaluated at w.
        """
        len = np.shape(w)[0]    #length of w
        regularization_term = np.sum(np.abs(w))
        regularization_term -= np.abs(w[len-1]) #deduct the last term
    ...
    def backward(self, w):
        ...
        len = np.shape(w)[0]   #length of w 
        gradient_term = np.zeros((len))
        for i in range(len-1):
            gradient_term[i] = (w[i]/np.absolute(w[i])) * self.reg_param
...
class L2Regularization(Regularization):
    ...
    def forward(self, w):
        ...
        regularization_term -= np.abs(w[len-1]**2) #deduct the last term
        regularization_term *= (self.reg_param*0.5)
        return regularization_term
    ...
    def backward(self, w):
        ...
        gradient_term = np.zeros((len))
        for i in range(len-1):
            gradient_term[i] = w[i] * self.reg_param
        return gradient_term
