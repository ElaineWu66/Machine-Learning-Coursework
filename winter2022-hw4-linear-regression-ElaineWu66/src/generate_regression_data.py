...
def generate_regression_data(degree, N, amount_of_noise=1.0):
    ...
    max_val_2, min_val_2 = 10, -10
    range_size_2 = (max_val_2 - min_val_2)  # 2
    coefficients = np.random.rand(degree+1) * range_size_2 + min_val_2
    ...
    for i in range(N):
        for power in range(dimension):
            y[i] += coefficients[power] * (x[i]**power)

    # add noise to y
    y_with_noise = y + np.random.normal(loc=0.0, scale=amount_of_noise*np.std(y), size=y.shape[0])

    return x,y_with_noise
    #raise NotImplementedError()
'''
x,y = generate_regression_data(5, 10, amount_of_noise=1.0)
print("x= ",x)
print("y= ",y)
'''