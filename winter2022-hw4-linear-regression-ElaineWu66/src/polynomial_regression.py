...
class PolynomialRegression():
    ...
    def fit(self, features, targets):
        ...
        #construct matrix X from features
        N = np.shape(features)[0]
        X = np.zeros((N,self.degree+1)) #initialization
        for i in range(N):
            for j in range(self.degree+1):
                X[i][j] = features[i]**j

        #construct x transpose
        X_tran = np.transpose(X)

        #construct self.parameters using the formula taught in class
        mul = np.dot(X_tran,X)
        mul = np.dot(np.linalg.inv(mul),X_tran)
        self.parameters = np.dot(mul,targets)
        #raise NotImplementedError()
    ...
    def predict(self, features):
        ...
        #make predictions
        predictions = np.dot(X,self.parameters)
    ...
    def visualize(self, features, targets,name):
        ...
        plt.plot(features, Y_predictions)
